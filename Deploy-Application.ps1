...
Try {
	...
	[string]$appScriptDate = '07/17/2019'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		## Show Welcome Message, close Minitab if needed, verify there is enough disk space to complete the install, and persist the prompt
		...
		## Uninstall Minitab 18
		If (Get-InstalledApplication -Name "Minitab 18") {
			$exitCode = Remove-MSIApplications -Name "Minitab 18" -PassThru
			If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
		}

		## Uninstall Minitab 17
		If (Get-InstalledApplication -Name "Minitab 17") {
			$exitCode = Execute-Process -Path "$envCommonProgramFilesX86\Minitab Shared\Minitab 17\Setup\Setup.exe" -Parameters "/p Minitab17 /x /qn" -WindowStyle "Hidden" -WaitForMsiExec -PassThru
			If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
		}

		## Uninstall Minitab 16
		If (Get-InstalledApplication -Name "Minitab 16") {
			$exitCode = Execute-Process -Path "$envCommonProgramFilesX86\Minitab Shared\Minitab 16\Setup\Setup.exe" -Parameters "/p Minitab16 /x /qn" -WindowStyle "Hidden" -WaitForMsiExec -PassThru
			If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
		}
		...
		$exitCode = Execute-Process -Path "$dirFiles/minitab19.1.1.0setup.x64.exe" -Parameters "/exenoui /exelang 1033 /qn ACCEPT_EULA=1 LICENSE_SERVER=vmwas22.winad.msudenver.edu LICENSE_SERVER_PORT=27090 DISABLE_UPDATES=1 DISABLE_ANALYTICS=1" -WindowStyle "Hidden" -WaitForMsiExec -PassThru
		If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
		...
		Remove-File -Path "$envCommonDesktop\Minitab 19.lnk"
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		## Show Welcome Message, close Minitab with a 60 second countdown before automatically closing
		Show-InstallationWelcome -CloseApps 'mtb,rmd' -CloseAppsCountdown 60
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBaUBvWYHghAv2S
# QOWhO0VBR4l1a+E5Mq2g3TlTk7y4dKCCFFwwggQUMIIC/KADAgECAgsEAAAAAAEv
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQgqc/gu95p
# dsZp7blJXfMeTu+zO9RqrFloQ/2Y1xYSBScwDQYJKoZIhvcNAQEBBQAEggEADiUL
# sf+tZKf/3Yy2CD0g43cQOTfsQiv0h45bXfeKkkuKcd1bZwqyOQ435HvIXqrM5eaD
# LtmsZj85ynsPtaZgocYJd/UmS3QWfy8vnJosLAk1mGT1nca8X+TORKA/v+oD12H4
# +dDFyxan4bKz+Lj6gRXp8eMp8n52YG7ilt+ca+xFkNWgdAukAZjiUW5JTT9jboAB
# 4otil0gblf79XSKqlkjbVm7wHceES/bZSMOG2NFu6BkXCQOCqJxhyrLOTNdaC9ZO
# wtuXkgK7qmHiiIRGrr63p4IaYmkR3Fh/XcSsiy+oY0+8a/tvVOgBSw2CZ14SCzft
# 9ql0kMZVD+mtRwsYPaGCAqIwggKeBgkqhkiG9w0BCQYxggKPMIICiwIBATBoMFIx
...
# CSqGSIb3DQEJBTEPFw0xOTA3MTcyMDA3MzZaMCMGCSqGSIb3DQEJBDEWBBRXEv8S
# wxk9pZfjMlfyICMbabtqeDCBnQYLKoZIhvcNAQkQAgwxgY0wgYowgYcwgYQEFGO4
...
# ggEAem7SJH6SLJVSW7EzLL+bCPlJD0Ox+DjvtMI5Y+2MpUDa76PbpsRDpdfzC9Ql
# 3LhKBWzJYhj+eMhYrbG6gkRO/gIGMOFCf3BQmFhG0oFCB4KQQYuHNFICWq6c1KvD
# KqfZz7VQFqslNmuaFu+EAvCoAQfT9xN0OBOmffefhrbDSsZ8Ws+lS8TW8chEA3tE
# QDc8pQUinW1ZVCJimrECYq65PsHcOq+llNMizMWrjRkcHSzMEcdn3yoMgtTfRJsT
# KezoXwz/E78xScQP0s5ITCWQQ5F+T2ZYgDpN+DyXU+WK89K86RfOMb3ePma+Q3yn
# FWaD+58+Ime/D/GqcVqFLuxFDA==
